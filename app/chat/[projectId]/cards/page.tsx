import Link from "next/link";
import { notFound } from "next/navigation";
import { getProject } from "@/app/actions/projects";
import { loadSrsState } from "@/app/actions/srs";
import { SrsRunner } from "@/components/srs-runner";

export default async function CardsPage({
  params,
}: {
  params: Promise<{ projectId: string }>;
}) {
  const { projectId } = await params;
  const project = await getProject(projectId);
  if (!project) notFound();
  const initial = await loadSrsState(projectId);

  return (
    <main className="mx-auto max-w-3xl px-6 py-10">
      <header className="mb-6">
        <Link href={`/chat/${projectId}`} className="text-xs text-muted-foreground hover:underline">
          ← back to chat
        </Link>
        <h1 className="text-2xl font-semibold tracking-tight">Spaced Repetition</h1>
        <p className="text-sm text-muted-foreground">{project.name} — {project.rootPath}</p>
      </header>
      <SrsRunner projectId={projectId} initial={initial} />
    </main>
  );
}
